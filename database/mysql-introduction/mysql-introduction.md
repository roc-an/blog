...
> 发布于 2021.04.26，最后更新于 2021.10.18。
...
插入一条记录时，可以不用所有字段都要有值，也就是说，对于一条记录可以仅插入部分字段。

但是这种时候在进行表的字段设计时一定要让 MySQL 知道该如何处理。对于没有赋值的字段，可以为空、设了默认值或是自增的。
...
PS：我把完整的查询语法也列出来，感兴趣可以深入了解：

```sql
SELECT *|字段
FROM 数据源（不一定是表，还可以是查询结果）
WHERE 条件
GROUP BY 字段
HAVING 条件
ORDER BY 字段
...
* `GROUP BY`：对查询结果分组，常与聚合函数配合使用；
* `HAVING`：筛选查询结果，和 `WHERE` 类似。
