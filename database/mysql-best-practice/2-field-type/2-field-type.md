...
ENUM | 枚举 | 值是预定的多个字符串值中的一个
SET | 字符串对象 | 值是预定的字符串值范围内的 0 或多个

`TEXT` 最灵活，因为它是按实际字符串长度来存储的，`TEXT` 又细分为 4 种，它们的区别是最大长度不同：

* `TINYTEXT`：最多 255 字符；
* `TEXT`：最多 65535 字符；
* `MEDIUMTEXT`：16777215 字符；
* `LONGTEXT`：4294967295 字符（约 4GB）

由于 `TEXT` 存储长度不确定，所以不能作为主键。
...
**`DATETIME` 是最常用的时间日期类型**，虽然它占的字节数较多，但却完整包含了年月日时分秒，这比将时间和日期拆开来存储和查询要方便得多。而且 `DATETIME` 的取值范围也最广，可以应对特殊场景。
...
* 朱晓峰老师的 [《MySQL 必知必会》](https://time.geekbang.org/column/intro/100073201?tab=catalog)
