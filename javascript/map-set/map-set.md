...
> 发布于 2021.10.10，最后更新于 2021.10.10。
...
因为 `WeakMap` 的键值对随时可能被销毁，所以没必要为它提供迭代键值对的能力，可能刚刚遍历完然后键值对就没了，没有什么参考意义。同样，`WeakMap` 也没有类似 `clear()` 一并清除所有键值对的能力。
...
ES6 新增的 `Set` 为 JS 这门语言带来了集合数据结构。

在数学中，集合具有“互异性”：一个集合中，任何两个元素都认为是不相同的，即每个元素只能出现一次。
...
理解了 `WeakMap`，那么 `WeakSet` 就简单很多了。

弱集合的“弱”，描述的是垃圾回收机制对待弱集合值的方式。`WeakSet` 中的值是“弱弱地拿着”，**这些值不属于正式的引用，不会阻止垃圾回收**。

弱集合中的值只能是 `Object` 或继承自 `Object` 的类型，尝试使用非对象设置值会抛出 `TypeError`。
...
console.log(ws.has(container.val)); // true

container.val = null;
console.log(ws.has(container.val)); // false
```
...
这篇文章中，我们重点讨论了 ES6 新增的数据类型 `Map` 和 `Set`，它们是 ECMAScript 中“集合引用类型”的一员。

`Map` 为 JS 带来了真正的键值对存储机制，是一种“值-值”映射。它有序，不存在意外的键，可以通过 `size` 属性轻松得到键值对个数，而且在性能方面通常要优于 `Object`。

`Set` 为 JS 带来了集合数据结构，它的成员具有唯一性。通过 `Set` 可以轻松实现数组去重、交集、并集、差集等特殊场景。

我们还在文中讨论了 JS 的垃圾回收机制：

JS 是自动进行垃圾回收的，最常用“标记-清除策略”。

标记-清除策略会遍历执行环境栈，过程中尝试访问其中的变量，并打上标记，区分出是“活跃数据”还是“垃圾数据”，最后再一并将垃圾数据清除。

另外一种“引用计数策略”由于存在循环引用问题，而被逐渐取代。

理解垃圾回收机制是理解 `WeakMap` 和 `WeakSet` 的关键，它们对待数据都是“弱引用”的。

`WeakMap` 的键以及 `WeakSet` 的值，都只能是对象，对于对象的引用不会阻止垃圾回收。

`WeakMap` 和 `WeakSet` 都不可迭代，因为其中的数据不确定什么时候就会被销毁调了。

`WeakMap` 和 `WeakSet` 常常结合 DOM 节点去用，来保存 DOM 节点的信息或是创建一个特定的集合。使用它们可以确保避免当 DOM 销毁后由于对应数据没清除而造成的的内存泄漏。

当然说起这种弱引用，ES2021 还推出了 [`WeakRef`](https://github.com/tc39/proposal-weakrefs)，可以直接创建对象的弱引用，由于目前市场上应用还比较少，这里就不再赘述了，感兴趣可以参考 [WeakRef | MDN](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/WeakRef)。

最后感谢收看，欢迎 `Watch` 和 `Star`，欢迎留言与我讨论:)
