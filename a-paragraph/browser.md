...
完成图层树（Layer Tree）构建后，渲染引擎会对图层树中的每个图层进行绘制

渲染引擎会把一个图层的绘制拆分成很多小的「绘制指令」，然后把这些指令按顺序组成一个「待绘制列表」。

可以在浏览器工具 -> Layer 中看到图层的详细信息和渲染过程分析

对于图层绘制阶段，输出的就是待绘制列表

### Step 6 - 栅格化（Raster）

绘制列表只是用来记录绘制顺序和绘制指令的列表，而实际的绘制操作是由渲染引擎中的「合成线程」来完成的

图层的绘制列表准备就绪后，主线程会把绘制列表**提交（Commit）**给合成线程

如果页面过大过长的话，用户仅能看到**视口（Viewport）**区域内的页面，这种情况下，如果要一次性渲染完所有图层，其实很多开销是没必要的

因此，**合成线程会将图层划分为图块（Tile）**，图块大小通常是 256x256，或者 512x512

之后，**合成线程会按照视口附近的图块来优先生成位图，实际生成位图的操作是由栅格化来执行的。所谓栅格化，就是指将图块转换为位图**

图块是栅格化执行的最小单位。渲染进程维护了一个栅格化的线程池，所有的图块栅格化都是在线程池内执行的

通常，栅格化过程都会使用 GPU 来加速生成，使用 GPU 生成位图的过程叫快速栅格化，或者 GPU 栅格化，生成的位图被保存在 GPU 内存中（这一步是渲染进程到 GPU 进程的跨进程操作）

### Step 7 - 合成和显示

一旦所有图块都被栅格化，合成线程就会生成一个绘制图块的指令“DrawQuad”提交给浏览器进程

随后，**浏览器进程会先将页面绘制到内存中，最后再显示到屏幕上**

### 渲染流程小结

1. 构建 DOM 树：将 HTML 文件转换为浏览器能理解的 DOM 树结构，即 `document`
2. 样式计算：将 CSS 文本转换为浏览器能理解的样式表，即 `document.styleSheets`，并计算出每个 DOM 节点的具体样式
3. 布局：构建布局树，并计算节点的布局信息
4. 分层：对布局树进行分层，生成分层树
5. 图层绘制：为每个图层生成绘制列表，并提交给合成线程
6. 栅格化：合成线程将图层分成图块，并将图块转换为位图
7. 合成和显示：浏览器进程生成页面，并显示在屏幕上
...
如果通过 JS 或 CSS **修改了元素的几何位置属性**，例如改变宽度、高度等，那么**浏览器会触发重新布局，从而导致重新解析之后的一系列子阶段**，这个过程就叫「重排」。

比如，`div.style.height = xxx` 就会触发重排

**重排需要更新完整的渲染流水线，所以开销是最大的**

### 重绘 - 更新元素的绘制属性

如果**修改了元素的绘制属性**（比如背景色 `div.style.background = xxx`），由于没有引起几何位置的改变，所以不会重新执行布局、分层（Layout、Layer），而是**重新进入绘制阶段（Paint）及其后续子阶段**，这个过程叫「重绘」

**相较于重排操作，重绘省去了布局和分层阶段，所以执行效率会比重排高一些**

### 直接合成阶段

如果更改一个既不需要布局、也不需要绘制的属性（比如 `transform: translate(xxx, xxx)`），渲染引擎将跳过布局、分层、绘制，只执行后续的合成操作，这个操作叫「合成」

合成操作也就是非主线程负责的子阶段：tiles -> raster -> draw quad -> display

比如使用 CSS 的 `transform` API 来实现动画效果，可以规避重排和重绘，直接在非主线程上执行合成动画操作。这样的效率是最高的

而且因为是在非主线程上合成，所以并没有占用主线程资源，另外页避开了布局、分层、绘制子阶段，所以**相对于重排和重绘，合成能大大提升绘制效率**
